...
class NaiveBayesClassifierSimulation
{
    ...
    public function classify(array $data) {
        ...
        foreach($data as $key => $datum) {
            if(array_key_exists($datum, $this->positiveGroups[$key]))
                $positive[] = ($this->positiveGroups[$key][$datum]+1) / (count($this->positives)+1);
            else
                $positive[] = 1;

            if(array_key_exists($datum, $this->negativeGroups[$key]))
                $negative[] = ($this->negativeGroups[$key][$datum]+1) / (count($this->negatives)+1);
            else
                $negative[] = 1;
        }
        $positive[] = (count($this->positives)+1) / (count($this->data)+2);
        $negative[] = (count($this->negatives)+1) / (count($this->data)+2);
...
$classifier = new NaiveBayesClassifierSimulation();
